    ...
    for (var 교시 = 1; 교시 <= 8; 교시++) {
        ...
        for (var 요일 = 1; 요일 < 7; 요일++) {
            ...
            if (dt < 제한일 || 제한일.getFullYear() < 2014 || isNaN(제한일)) {
                원자료 = 자료.자료254[학년][반][요일][교시];
                일일자료 = 자료.자료169[학년][반][요일][교시];
                if (자료.강의실 == 1) {
                    강의실 = 자료.자료718[학년][반][요일][교시];
                    if (강의실 > 0) {
                        강의실 = '<br>' + 강의실;
                    } else {
                        강의실 = '';
                ...
                if (일일자료 > 100) {
                    var 성명 = "";
                    if (th < 자료.자료368.length) {
                        성명 = 자료.자료368[th].substr(0, 2);
                    }
                    p += "<td class='" + 속성 + "'>" + 자료.자료564[sb] + "<br>" + 성명 + 강의실 + "</td>";
                } else {
                    p += "<td class='" + 속성 + "'></td>";
                }
            } else {
...
function sc_data(sc, sc2, r) {
    ...
    var s7 = sc + sc2;
    var sc3 = './117948?' + btoa(s7 + '_' + da1 + '_' + r);
    $.ajax({
        url: sc3,
...
function ba_change() {
    ...
    var 반 = Number(m2[1]);
    storage.ba = m;
    $('#hour').empty().append(자료155(H시간표, 학년, 반));
}
